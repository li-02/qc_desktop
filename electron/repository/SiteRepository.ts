...
export class SiteRepository {
  ...
  getAllSites(): ServiceResponse<Site[]> {
    try {
      const stmt = this.db.prepare('SELECT * FROM sys_site ORDER BY created_at DESC');
  ...
  getSiteById(id: number): ServiceResponse<Site> {
    try {
  ...
  createSite(site: Omit<Site, 'id' | 'created_at' | 'updated_at'>): ServiceResponse<number> {
    try {
  ...
  updateSite(id: number, site: Partial<Omit<Site, 'id' | 'created_at' | 'updated_at'>>): ServiceResponse<void> {
    try {
  ...
  deleteSite(id: number): ServiceResponse<void> {
    try {
