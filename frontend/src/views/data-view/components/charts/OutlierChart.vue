...
const initScatterChart = () => {
  ...
  // 过滤当前选中列的数据
  const currentDetails = props.details.filter(d => (d.column_name || (d as any).columnName) === selectedColumn.value);
    ...
    series: [
      {
        ...
        markLine: {
          ...
          data: [
            ...(currentThresholds.value?.minThreshold != null
              ? [
                  {
                    xAxis: currentThresholds.value.minThreshold,
                    name: "下限",
                    lineStyle: { color: "#ef4444", type: "solid" as const, width: 2 },
                  },
                ]
              : []),
            ...(currentThresholds.value?.maxThreshold != null
              ? [
                  {
                    xAxis: currentThresholds.value.maxThreshold,
                    name: "上限",
                    lineStyle: { color: "#ef4444", type: "solid" as const, width: 2 },
                  },
                ]
    ...
    <div v-if="showDetailChart !== false && details.length > 0" class="chart-card">
      <div class="chart-header">
        ...
        <div class="chart-actions">
          ...
          <el-select v-model="selectedColumn" placeholder="选择指标" size="small" style="width: 150px">
            <el-option
