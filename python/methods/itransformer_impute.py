"""
iTransformer model inference script for missing-value imputation.

Loads a PyPOTS iTransformer checkpoint plus the metadata produced by the local
training script, applies the saved scaler, imputes windowed sequences, and
writes the original CSV back with only the requested target gaps filled.
...
def infer_metadata_path(model_path: str) -> str:
    ...
    raise FileNotFoundError(
        f"未找到 iTransformer metadata 文件，已尝试: {', '.join(str(c) for c in candidates)}"
    )
...
def create_windows(data: np.ndarray, seq_len: int) -> np.ndarray:
    ...
    if n_windows <= 0:
        raise ValueError(f"数据长度 ({n_samples}) 小于 iTransformer 窗口长度 ({seq_len})")
...
def infer_dimensions_from_seq_len(seq_len: int, n_heads: int) -> tuple[int, int, int, int]:
    if seq_len <= 192:
        d_model = 128
    elif seq_len <= 768:
        d_model = 256
    ...
    d_k = d_model // n_heads
    d_v = d_model // n_heads
    return d_model, d_ffn, d_k, d_v


def build_model(args: argparse.Namespace, n_steps: int, n_features: int) -> iTransformer:
    d_model, d_ffn, d_k, d_v = infer_dimensions_from_seq_len(n_steps, args.n_heads)
    d_model = args.d_model or d_model
    d_ffn = args.d_ffn or d_ffn
    d_k = args.d_k or d_model // args.n_heads
    d_v = args.d_v or d_model // args.n_heads
...
def run(args: argparse.Namespace) -> None:
    ...
    working_df = ensure_model_columns(
        df=df,
    ...
    if missing_features:
        raise ValueError(f"数据缺少 iTransformer 模型所需特征列: {', '.join(missing_features)}")

    print(f"[2/5] 正在加载 iTransformer 模型: {args.model}", flush=True)
    model = build_model(args, seq_len, len(feature_names))
    model.load(args.model)

    print("[3/5] 正在构建 iTransformer 输入序列...", flush=True)
    ...
    print("[4/5] 正在执行 iTransformer 插补...", flush=True)
    imputed_batches = []
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="iTransformer 缺失值插补推理脚本",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    parser.add_argument("--model", required=True, help="训练好的 iTransformer .pypots 模型路径")
    parser.add_argument("--metadata", default="", help="训练时保存的 metadata.joblib 路径")
    ...
    parser.add_argument("--batch-size", type=int, default=4, help="推理批次大小")
    ...
    parser.add_argument("--d-k", type=int, default=0)
    parser.add_argument("--d-v", type=int, default=0)
    parser.add_argument("--d-ffn", type=int, default=0)
    parser.add_argument("--dropout", type=float, default=0.1)
    parser.add_argument("--attn-dropout", type=float, default=0.0)
    parser.add_argument("--ort-weight", type=float, default=1.0)
    parser.add_argument("--mit-weight", type=float, default=1.0)
