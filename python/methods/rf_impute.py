"""
随机森林模型推理脚本（缺失值插补）
功能：加载训练好的随机森林模型（.pkl），对含缺失值的数据进行插补

使用示例:
    python rf_impute.py \
        --model "models/NEE/rf_nee.pkl" \
        --file "input.csv" \
        --target "NEE" \
...
def build_feature_matrix(
    ...
    if use_time_features:
        ...
        for feat in ["sin_doy", "cos_doy", "sin_hour", "cos_hour"]:
...
def run(args: argparse.Namespace) -> None:
    ...
    # 2. 加载模型
    print(f"[2/5] 正在加载模型: {args.model}", flush=True)
    ...
    # 4. 执行插补
    print("[4/5] 正在执行随机森林插补...", flush=True)
    ...
    try:
        ...
        raise RuntimeError(f"模型预测失败: {exc}") from exc
