"""
TimeMixer model inference script for missing-value imputation.

Loads a PyPOTS TimeMixer checkpoint plus the saved training metadata, applies
the saved scaler, imputes windowed sequences,
...
def infer_metadata_path(model_path: str) -> str:
    ...
    raise FileNotFoundError(
        f"未找到 TimeMixer metadata 文件，已尝试: {', '.join(str(c) for c in candidates)}"
    )
...
def create_windows(data: np.ndarray, seq_len: int) -> np.ndarray:
    ...
    if n_windows <= 0:
        raise ValueError(f"数据长度 ({n_samples}) 小于 TimeMixer 窗口长度 ({seq_len})")
...
def build_model(args: argparse.Namespace, n_steps: int, n_features: int) -> TimeMixer:
    return TimeMixer(
        n_steps=n_steps,
        ...
        channel_independence=parse_bool(args.channel_independence, False),
        ...
        apply_nonstationary_norm=parse_bool(args.apply_nonstationary_norm, False),
...
def run(args: argparse.Namespace) -> None:
    ...
    if missing_features:
        raise ValueError(f"数据缺少 TimeMixer 模型所需特征列: {', '.join(missing_features)}")

    print(f"[2/5] 正在加载 TimeMixer 模型: {args.model}", flush=True)
    model = build_model(args, seq_len, len(feature_names))
    model.load(args.model)

    print("[3/5] 正在构建 TimeMixer 输入序列...", flush=True)
    ...
    print("[4/5] 正在执行 TimeMixer 插补...", flush=True)
    imputed_batches = []
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="TimeMixer 缺失值插补推理脚本",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    parser.add_argument("--model", required=True, help="训练好的 TimeMixer .pypots 模型路径")
    ...
    parser.add_argument("--d-ffn", type=int, default=32)
    parser.add_argument("--top-k", type=int, default=5)
    parser.add_argument("--dropout", type=float, default=0.1)
    parser.add_argument("--channel-independence", default="false")
    parser.add_argument("--decomp-method", default="moving_avg")
    parser.add_argument("--moving-avg", type=int, default=25)
    parser.add_argument("--downsampling-layers", type=int, default=3)
    parser.add_argument("--downsampling-window", type=int, default=2)
    parser.add_argument("--apply-nonstationary-norm", default="false")
