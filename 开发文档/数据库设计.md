...
| 前缀 | 含义 | 示例 |
|------|------|------|
| `sys_` | 系统基础表 | `sys_site`, `sys_dataset` |
...
| `description` | TEXT | - | NULL | 站点描述 |
| `latitude` | REAL | - | NULL | 纬度（-90 ~ 90） |
| `longitude` | REAL | - | NULL | 经度（-180 ~ 180） |
| `altitude` | REAL | - | NULL | 海拔高度（米） |
...
**业务说明**：
- 站点是系统的顶层管理单元，代表实际的数据采集地点
- 支持记录地理位置信息，便于空间分析和可视化
...
| `site_id` | INTEGER | NOT NULL, FK | - | 所属站点ID |
...
```
全局配置(APP) → 站点配置(SITE) → 数据集配置(DATASET)
     ↓              ↓                  ↓
   优先级低      优先级中           优先级高
```
...
2. **设置 WAL 模式**：提升并发性能
3. **初始化核心表**：`sys_site`, `sys_dataset`, `conf_column_setting`, `biz_dataset_version`, `stat_version_detail`
...
SELECT d.* 
FROM sys_dataset d
WHERE d.site_id = ? AND d.is_del = 0
...
WHERE is_del = 0 AND is_active = 1
  AND (
    (scope_type = 'APP') OR
    (scope_type = 'SITE' AND scope_id = ?) OR
